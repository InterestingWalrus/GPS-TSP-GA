...
/// @returns nothing
void GpsClass::swap(std::vector<QGeoCoordinate> &gps_list, int i, int j)
{
    QGeoCoordinate temp = gps_list[i];
...
/// @param j second position to be swapped
/// @returns nothing
 void GpsClass::swap(std::vector<int>&route, int i, int j)
 {
     int temp = route[i];
     route[i] = route[j];
     route[j] = temp;
...
/// @returns nothing
 void GpsClass::shuffle(std::vector<int> &route)
 {
 ...
 double GpsClass::GetRouteDistance(std::vector<QGeoCoordinate>&gps_list, std::vector<int>&order)
 {
     double sum = 0;
 ...
 /// @returns nothing
 void GpsClass::normalizeFitness()
 {
     double sum = 0;
 ...
 /// @returns nothing
 void GpsClass::initialiseGA(std::vector<QGeoCoordinate>& wp, int MAX_POP )
 {
     waypoints = wp;
...
/// inserted into the next generation of population
 ...
 std::vector<int> GpsClass::pickRandomParent(std::vector<std::vector<int> > &population, std::vector<float>& fitness)
 {
     int index = 0;
 ...
 /// @returns nothing
 void GpsClass::mutate(std::vector<int> &order, float mutationRate)
 {
     for(std::size_t i = 0; i < waypoints.size(); i++)
     {
        double r = ((double) rand() / (RAND_MAX));
 ...
 /// @see nextGeneration()
 std::vector<int> GpsClass::crossover(std::vector<int> &parentA, std::vector<int> &parentB)
 {
