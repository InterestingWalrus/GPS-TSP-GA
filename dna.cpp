...
DNA::DNA(std::vector<QGeoCoordinate> &wp, int num_drones)
...
DNA::DNA(std::vector<std::size_t> &ord, std::vector<QGeoCoordinate>& wp, int num_drones)
      :bestOrder(num_drones, std::vector<std::size_t>(0))
{
...
void DNA::computeFitness()
{
    ...
    for(std::size_t i = 0; i < number_of_drones; ++i)
    {
        ...
        std::vector<QGeoCoordinate> route_waypoint;
...
void DNA::setWaypoints(std::vector<QGeoCoordinate>& wp)
{
    number_of_cities = wp.size();
    waypoints = wp;
}
